...
#define GPS_DBG_FUNC(fmt, arg ...)    printk(KERN_INFO PFX "%s: " fmt, \
__FUNCTION__, \
## arg)

#define GPS_ERR(fmt, arg ...)         printk(KERN_ERR PFX "%s: " fmt, \
__FUNCTION__, \
## arg)
#define GPS_WARN(fmt, arg ...)        printk(KERN_WARNING PFX "%s: " fmt, \
__FUNCTION__, \
## arg)
#define GPS_NOTICE(fmt, arg ...)      printk(KERN_NOTICE PFX "%s: " fmt, \
__FUNCTION__, \
## arg)
#define GPS_INFO(fmt, arg ...)        printk(KERN_INFO PFX "%s: " fmt, \
__FUNCTION__, \
## arg)
...
static char *str_reason[] =
    { "none", "init", "monitor", "wakeup", "TTFF", "force", "unknown" };
/******************************************************************************
* Functions
******************************************************************************/
					...
					const char *buf, size_t count)
{
	...
	if (sscanf(buf, "(%d/%d/%d %d:%d:%d) - %d/%d", &year, &mon, &day,
		...
		int number =
		    (int)(sizeof(obj->status.items) /
			  sizeof(obj->status.items[0]));
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	for (idx = 0; idx < num; idx++) {
		...
		cnt = snprintf(buf + len,
			       ...
			       idx,
			       obj->status.items[idx].year + 1900,
...
DEVICE_ATTR(pwrctl,
	    ...
	    mt3326_show_pwrctl, mt3326_store_pwrctl);
DEVICE_ATTR(suspend,
	    S_IWUSR | S_IWGRP | S_IRUGO,
	    mt3326_show_suspend, mt3326_store_suspend);
DEVICE_ATTR(status,
	    S_IWUSR | S_IWGRP | S_IRUGO,
	    mt3326_show_status, mt3326_store_status);
DEVICE_ATTR(state,
	    S_IWUSR | S_IWGRP | S_IRUGO, mt3326_show_state, mt3326_store_state);
DEVICE_ATTR(pwrsave,
	    S_IWUSR | S_IWGRP | S_IRUGO,
	    mt3326_show_pwrsave, mt3326_store_pwrsave);
#if defined(GPS_CONFIGURABLE_RESET_DELAY)
DEVICE_ATTR(rdelay,
	    S_IWUSR | S_IWGRP | S_IRUGO,
	    mt3326_show_rdelay, mt3326_store_rdelay);
#endif
...
static int mt3326_gps_ioctl(struct inode *inode,
			    struct file *file,
...
#else
long mt3326_gps_unlocked_ioctl(struct file *file,
			       unsigned int cmd, unsigned long arg)
{
...
/******************************************************************************/
static ssize_t mt3326_gps_read(struct file *file,
...
/******************************************************************************/
static ssize_t mt3326_gps_write(struct file *file,
...
static int mt3326_gps_probe(struct platform_device *dev)
{
	...
	devobj->dev = device_create(devobj->cls,
				    NULL, devobj->devno, drvobj, "gps");
	drvobj->hw = hw;
	drvobj->pwrctl = 0;
	drvobj->suspend = 0;
